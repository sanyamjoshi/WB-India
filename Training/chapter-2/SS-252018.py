...
def euler_system(bvec, *args):
    ...
    euler_errors = u_prime(cvec[:2], sigma) - beta * (1 + r) * u_prime(cvec[1:], sigma)
    return euler_errors


bvec_init = np.array([0.01, 0.01])
args = (nvec, beta, sigma, A, alpha, delta)
results = opt.root(euler_system, bvec_init, args=(args))
b_ss = results.x
